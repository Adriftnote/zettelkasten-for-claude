...
CNN은 시각피질을 의도적으로 모방하여 설계되었으므로, GPU의 병렬 처리와 뇌 시각피질의 병렬 처리는 기본 연산 패턴이 유사하다. 입력 → 가중치 곱셈 → 활성화 함수 → 출력이라는 단위 연산이 유사하고, 이것을 대규모로 동시 실행하는 구조도 닮아 있다. 다만 실제 뉴런은 인공 뉴런보다 훨씬 복잡하다.
...
- [fact] 뉴런 연산(입력→가중치→활성화→출력)과 딥러닝 연산(입력→행렬곱→활성화→출력)의 기본 패턴이 유사하다 (실제 뉴런은 더 복잡함) #computation
