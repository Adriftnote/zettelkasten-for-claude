...
- connects_to [[컨텍스트 엔지니어링 (Context Engineering)]] (컨텍스트 최적화 기법)
- connects_to [[MCP Tool 패턴 (MCP Tool Patterns)]] (도구 로딩 최적화)
- connects_to [[아키텍처 (Architectures)]] (아키텍처 수준 최적화)
- connects_to [[메모리 시스템 (Memory Systems)]] (KV 캐시와 메모리 계층)
- connects_to [[일반 CS 개념 (General CS)]] (기본 알고리즘과 트레이드오프)