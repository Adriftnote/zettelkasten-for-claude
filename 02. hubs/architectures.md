...
- [insight] 아키텍처는 변경하기 어려운 결정이다. 처음에 잘 설계해야 나중에 고생하지 않는다 #system-design
- [insight] 도구, 지식, 에이전트의 3계층 아키텍처 필요 #architecture
- [insight] 중앙 집중식 관리가 확장성과 유지보수성을 결정 #tool-management
- [path] 기초 (철학) → 구현 (패턴) → 최적화 (선택 가이드) #learning
- [path] 도구관리 → 지식관리 → 에이전트설계 #learning
