...
- 프로그래밍의 본질은 **합성(Composition)** - 작은 조각을 조합해 큰 프로그램을 만든다 #category-theory
- **타입 = Object, 함수 = Morphism** - 타입 시스템이 곧 카테고리 #type-system
- Functor/Monad는 "컨테이너 패턴"의 수학적 일반화 - Option, Result, Vec 모두 Functor #functor
- Rust의 `?` 연산자는 Haskell의 do-notation과 동치 - 이미 모나딕 프로그래밍 지원 #monad
- 불가능한 상태를 타입으로 제거 - Sum Type(enum)의 진정한 힘 #adt
- **Monad = Monoid in category of endofunctors** - 모든 것은 합성으로 귀결 #monad
- Quick Start: Category → Types → Functors → Monads (실용 중심) #learning
- Theoretical: Category → Products → Natural Trans. → Limits → Yoneda → Adjunctions #learning
- Rust 실전: 챕터별 → 개념별 → 프로젝트 기반 #rust
