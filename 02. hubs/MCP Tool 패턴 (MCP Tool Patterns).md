...
- [fact] 도구가 많아지면 LLM은 선택 장애를 겪는다 - 필요한 도구만 보여주는 것이 핵심 #mcp #tool
- [fact] 도구는 구조화된 인덱싱으로 관리해야 스케일링 가능해진다 #indexing #scalability
- [fact] Hook 기반 자동활성화는 수동 관리의 복잡성을 제거한다 #automation #hook
...
- connects_to [[컨텍스트 엔지니어링 (Context Engineering)]] (도구 설명도 컨텍스트의 일부)
  - connects_to [[최적화 패턴 (Optimization Patterns)]] (토큰 최적화 기법)
- connects_to [[아키텍처 (Architectures)]] (MCP Tool Hub 아키텍처)