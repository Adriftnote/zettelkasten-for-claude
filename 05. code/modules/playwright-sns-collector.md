...
## 시청시간 일괄 수집 실전 결과 (2026-03-26)

collect-watch-time 스크립트 시리즈로 4개 플랫폼 전체 영상 시청시간 수집 완료. 167행 Excel 리포트 생성(`data/watch-time/watch_time_report.xlsx`).

### YouTube: get_cards가 핵심 (get_screen 아님)

- [note] **`get_screen`에는 metricType/metricColumns가 없음** — 위 "수집 가능 지표" 표는 실제로는 `get_cards`의 `audienceRetentionHighlightsCardData`에서 나옴 #correction
- [impl] 정확한 경로: `get_cards → cards[N].audienceRetentionHighlightsCardData.videosData[0].metricTotals` #algo
  - `avgViewDurationMillis` — 평균 시청 시간 (ms)
  - `avgPercentageWatched` — 평균 조회율 (1.0 = 100%)
  - `views` — 조회수
  - 총 시청시간 = views × avgViewDurationMillis로 계산
- [note] **Shorts 루프 재생 주의**: `period-since_publish`(게시 이후) 기간에서 avgPercentageWatched 100%+ 영상 17/50개. `get_cards`는 URL 기간 파라미터(`period-default` 등) 무시하고 항상 전체 기간 반환 #caveat
- [note] Shorts 콘텐츠 품질 지표는 avgWatchTime보다 **계속시청률**(고급 모드에서 확인)이 더 의미 있음 #context
- 스크립트: `collect-yt-watchtime.js` (49/50 성공)

### TikTok: API 직접호출 vs 페이지 캡처 wrapper 차이

- [impl] `/aweme/v2/data/insight/` API에 `insigh_type` 추가하여 시청시간 수집 #algo
  - `video_total_duration_realtime` — 총 시청시간 (초)
  - `video_per_duration_realtime` — 평균 시청시간 (초)
  - `video_finish_rate_realtime` — 완시율
  - `realtime_total_video_views` — 총 조회수
- [note] **API 직접호출 시 일부 insigh_type이 null로 반환** (14/45 누락) — 페이지 접속(`/tiktokstudio/analytics/{itemId}`) 방식으로 fallback 필요 #caveat
- [note] **응답 wrapper 구조 다름**: API 직접호출=`{status, value: 숫자}`, 페이지 캡처=`{value: {status, value: 숫자}}` 이중 wrapper #caveat
- 스크립트: `collect-tiktok-watchtime.js` + `collect-tiktok-missing.js` (45/45 성공)

### Meta FB/IG: entity_type으로 구분 (순서 아님)

- [impl] `TofuObjectInsightsV2EntityQuery` 응답에서 `entity_type` 필드로 FB/IG 구분 #algo
  - `FB_PAGE_POST` → Facebook 지표
  - `IG_POST` → Instagram 지표
- [note] **query_context 카운트 순서 기반 구분은 틀림** — 응답 순서: FB → IG → FB(+FOA) 3세트이지만 entity_type이 유일한 정확한 방법 #correction
- [impl] 시청시간 지표: `VIDEO_VIEW/TIME` (총시청시간 ms), `AVG_VIDEO_VIEW/TIME` (평균시청시간 ms), `VIEW/COUNT` (조회수) #algo
- [note] **리텐션 커브**: FB만 `useTofuMetricsQuery` + `RETENTION_BUCKET` breakdown으로 초별 유지율 제공. IG는 Business Suite에서 미제공 (앱에서만 확인 가능) #caveat
- 스크립트: `collect-meta-watchtime.js` (36/39 성공, 3개 텍스트 게시물)

### 수집 교훈

- [note] **1개 영상으로 API 구조 확인 후 전체 수집** — 구조 추측으로 전체 돌리면 재실행 반복 #pattern
- [note] 릴스 필터: DB에서 `post_title NOT LIKE '- %'`로 텍스트 게시물 제외 (정확도 ~92%) #pattern
