...
- [impl] 2개 스크립트 분리: `run.js`(채널 일별 요약) / `run-posts.js`(게시물별 스냅샷) #pattern
...
```
playwright-test/
├── package.json         ← scripts: setup/test/run/collect/collect-posts
├── setup.js             ← 최초 로그인 세션 저장 (1회)
├── test-session.js      ← 세션 유지 확인 테스트
├── run.js               ← 채널 일별 요약 수집 (Facebook, X, Naver Blog)
├── run-posts.js         ← 게시물별 조회수 스냅샷 (NaverTV, Naver Blog, Meta FB/IG)
...
|------|-----------------|------------|
| 자동화 | ❌ activeTab 수동 클릭 필요 | ✅ 완전 자동 (cron 가능) |
...
| 네이버TV | apis.naver.com clips API | 최근 10개 클립 totalPlayCount |
| 네이버 블로그 | admin.blog iframe 순위 테이블 | 당일 조회수 상위 게시물 |
| Meta | GraphQL doc_id 34106016262374963 (FB+IG 통합) | 최근 10개 게시물 views |
| TikTok | `/creator/manage/item_list/v1` API 가로채기 | 최근 10개 게시물 play_count |
...
| 네이버TV | apis.naver.com clips API | 최근 10개 클립 totalPlayCount |
| 네이버 블로그 | admin.blog iframe 순위 테이블 | 당일 조회수 상위 게시물 |
| Meta FB | GraphQL doc_id 34106016262374963 | 최근 10개 게시물 views |
| Meta IG | 동일 GraphQL (igId 기반) | 최근 10개 게시물 views |
...
- contains [[run-posts-collector]] (게시물별 조회수 스냅샷)
- contains [[playwright-setup]] (세션 초기화)
- contains [[playwright-test-session]] (세션 확인)
