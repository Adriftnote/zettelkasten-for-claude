...
- [note] YouTube: `page.on('response')` 로 `list_creator_videos` API 가로채기 — DOM 파싱(ytcp-video-row)에서 전환 #context
- [note] YouTube: 동영상(`videos/upload`) + Shorts(`videos/short`) 탭 순차 수집 → Set으로 중복 제거 #algo
- [note] YouTube: 다음 버튼(`aria-label="다음"|"Next"|#navigate-after`) 클릭으로 페이지네이션, totalSize 도달 시 중단 #context
...
|--------|---------|---------|--------|
| youtube | response 가로채기 | `list_creator_videos` API | videoId, title, externalViewCount (동영상+Shorts 전체 페이지) |
...
| meta | page.evaluate 내부 GraphQL | doc_id 34106016262374963 | row_id, title, views (FB+IG 통합 반환) |
...
- contains [[run]]
  ...
  - run calls [[get-captured-at]] (line 514)
  - run calls [[collect-you-tube]] (line 534)
  - run calls [[collect-naver-tv]] (line 545)
  - run calls [[collect-naver-blog]] (line 556)
  - run calls [[collect-meta]] (line 567)
  - run calls [[collect-tik-tok]] (line 578)
  - run calls [[send-to-webhook]] (line 591)
