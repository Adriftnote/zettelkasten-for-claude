...
- [impl] 이미 변환된 HTML 행(table/tr/td/div.editor-table-wrap 시작)은 그대로 통과시켜 이중 변환 방지 #pattern
- [impl] 리스트 연속성: 빈 줄이 리스트 스택 안에 있으면 무시(리스트 분리 방지), 바깥에서는 `<div>&nbsp;</div>` emit #algo
- [impl] 헤딩(h1~h3)은 Flow 네이티브 방식으로 태그만 출력 (인라인 스타일 없음) #pattern
...
입력: MD 문자열
    ↓
[1] convertCodeBlocks()     ─── ```lang\n...\n``` → editor-table-wrap 1칸 테이블
    ↓
[2] convertTables()         ─── | col | ... | 행 그룹 → editor-table-wrap 테이블
    ↓
[3] split('\n') 라인 순회
    │
    ├─ <div.editor-table-wrap / <table / <tr / <td / <tbody 시작 행  → 그대로 통과
    ├─ --- / *** / ___ (3+)     → <hr>
    ├─ ### / ## / #             → <h3>/<h2>/<h1> + applyInlineStyles()
    ├─ 숫자 리스트 N. text      → <ol><li> (스택 관리)
    ├─ 불릿 - text             → <ul><li> (스택 관리)
    ├─ 빈 줄                    → 리스트 안이면 skip / 밖이면 <div>&nbsp;</div>
...
- calls [[convert-code-blocks]] (line 301)
- calls [[convert-tables]] (line 304)
- called_by [[create-subtask-api]] (line 578)
- called_by [[create-task-api]] (line 514)
